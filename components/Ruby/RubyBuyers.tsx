...
import { Gem, Store, Crown, Landmark, UserCircle } from "lucide-react";
...
export default function RubyBuyers() {
  return (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          <ClientAnimations delay={0.2} className="lg:col-span-2">
            <div className="group h-full bg-gradient-to-br from-white via-[#fffaf0] to-white border border-[#c78b2e]/20 p-6 rounded-2xl transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_15px_40px_rgba(199,139,46,0.25)] hover:border-[#c78b2e]/50 relative overflow-hidden flex flex-col justify-center">
              ...
              <Store className="w-10 h-10 text-[#c78b2e] mb-5 transition-transform duration-300 group-hover:scale-110" />
              <h3 className={`${montserrat.className} text-xl font-extrabold text-[#0a0a0a]`}>
                Gem dealers and gemstone wholesalers
              </h3>
            </div>
          </ClientAnimations>

          {[
            { icon: Gem, title: "Fine jewelry manufacturers" },
            { icon: Crown, title: "Luxury goods brands sourcing colored stones" },
            { icon: Landmark, title: "Auction houses and estate jewelry specialists" },
            { icon: UserCircle, title: "Private collectors and high-net-worth buyers" },
          ].map((item, i) => (
